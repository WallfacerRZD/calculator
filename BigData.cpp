...
BigData DoAdd(const BigData &lhs, const BigData &rhs) {					//正加正
	int carry = 0;
...
BigData DoSubtraction(const BigData &lhs, const BigData &rhs) {		//正(大) - 正(小)
	auto lhs_beg = lhs.data_.crbegin();
	auto rhs_beg = rhs.data_.crbegin();
	auto lhs_end = lhs.data_.crend();
	auto rhs_end = rhs.data_.crend();
...
BigData DoMultiply(const BigData &lhs, const BigData &rhs) {		//正(多位)*正(一位)
	auto lhs_beg = lhs.data_.crbegin();
	auto lhs_end = lhs.data_.crend();
	auto rhs_beg = rhs.data_.crbegin();
	int carry = 0;
